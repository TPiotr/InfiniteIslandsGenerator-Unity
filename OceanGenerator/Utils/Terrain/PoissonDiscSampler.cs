...
public class PoissonDiscSampler
{
    ...
    ///   foreach (Vector2 sample in sampler.Samples()) { ... }
    public IEnumerable<Vector2> Samples(int seed)
    {
        random = new System.Random(seed);

        // First sample is choosen randomly
        yield return AddSample(new Vector2(value() * rect.width, value() * rect.height));

        while (activeSamples.Count > 0) {

            // Pick a random active sample
            int i = (int) value() * activeSamples.Count;
            ...
            for (int j = 0; j < k; ++j) {
                ...
                float angle = 2 * Mathf.PI * value();
                float r = Mathf.Sqrt(value() * 3 * radius2 + radius2); // See: http://stackoverflow.com/questions/9048095/create-random-number-within-an-annulus/9048443#9048443
